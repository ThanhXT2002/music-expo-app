...
function AuthInput({
  ...
  autoCapitalize = 'none', hasError = false,
}: {
  value: string; onChangeText: (t: string) => void;
  placeholder: string; icon: React.ReactNode;
  secureEntry?: boolean; keyboardType?: any;
  autoCapitalize?: any; hasError?: boolean;
}) {
...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    if (!displayName.trim() || !email.trim() || !password || !confirmPassword) { setError('Vui lòng điền đầy đủ thông tin'); return; }
    ...
    try {
      ...
      router.replace('/(tabs)');
    } catch { setError('Đăng ký thất bại. Email đã được sử dụng.'); }
  };
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.flex}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.form}>
          <AuthInput value={displayName} onChangeText={setDisplayName} placeholder="Tên hiển thị" autoCapitalize="words" icon={<User size={18} color={COLORS.textMuted} />} />
          ...
          <AuthInput value={confirmPassword} onChangeText={setConfirmPassword} placeholder="Xác nhận mật khẩu" secureEntry hasError={mismatch} icon={<Lock size={18} color={mismatch ? COLORS.error : COLORS.textMuted} />} />
          ...
          <Pressable onPress={handleRegister} disabled={isLoading}>
            <View style={styles.primaryBtn}>
              {isLoading ? <ActivityIndicator color="#fff" /> : (
                <View style={styles.primaryBtnInner}><Text style={styles.primaryBtnText}>Đăng ký</Text><ChevronRight size={18} color="#fff" /></View>
              )}
            </View>
          </Pressable>
        </View>
        ...
        <Text style={styles.terms}>
          ...
          <Text style={styles.termsLink} onPress={() => router.push('/auth/terms-of-service')}>Điều khoản dịch vụ</Text>
          {' '}và{' '}
          <Text style={styles.termsLink} onPress={() => router.push('/auth/privacy-policy')}>Chính sách bảo mật</Text>
        </Text>
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 14, color: COLORS.textSecondary, textAlign: 'center', marginBottom: 24, fontStyle: 'italic', letterSpacing: 0.3 },
